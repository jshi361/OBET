...
def before_request():
    # If user is not authenticated redirect them to "unconfirmed" page, otherwise continue with their request
    if current_user.is_authenticated() and not current_user.confirmed and str(request.endpoint)[:5] != 'auth.':
